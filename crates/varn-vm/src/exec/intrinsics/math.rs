...
pub fn dispatch(op: u8, args: &[VmValue]) -> VmResult<VmValue> {
    // args[0] is the Math module object (dest); real numeric args start at args[1]
    ...
    let result = match op {
        ...
        v if v == MathOp::Min as u8 => {
            args[2..].iter().fold(x, |acc, v| acc.min(v.to_f64()))
        }
        v if v == MathOp::Max as u8 => {
            args[2..].iter().fold(x, |acc, v| acc.max(v.to_f64()))
        }
        _ => return Ok(VmValue::null()),
    };
    if arg1.is_int() && result.fract() == 0.0 && result >= i64::MIN as f64 && result <= i64::MAX as f64 {
