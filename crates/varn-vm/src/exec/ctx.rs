...
use varn_types::{FunctionProto, Literal, NativeCtx, PoolEntry, Value, ModuleObj};
...
impl ExecCtx {
    ...
    pub fn convert_to_module_obj(&mut self, id: ModuleId, val: VmValue) -> VmResult<VmValue> {
        ...
        match self.heap.get(raw_idx) {
            Some(crate::heap::HeapObj::Module(_)) => {
                Ok(val)
            }
    ...
    pub fn load_module(&mut self, specifier: &str) -> VmResult<VmValue> {
        ...
        if let ModuleId::Stdlib(ref name) = resolved {
            if let Some(nv) = varn_builtins::build_module(name, &mut self.heap) {
                ...
                self.modules.insert(resolved.as_str().to_string(), converted);
                return Ok(converted);
            }
        }
        ...
        if let Some(proto) = self.precompiled.get(&resolved.as_str()).cloned() {
            ...
            module_obj.export_map = export_map;
            let module_val = self.heap.alloc_module(std::rc::Rc::new(module_obj));
            self.modules.insert(resolved.as_str().to_string(), module_val);
                ...
                if brace_depth == 0 && (c.is_alphabetic() || c == '_') {
                    ...
                    if word == "export" {
                        ...
                        if keyword_idx < words.len() {
                            ...
                            if ["function", "class", "interface", "namespace", "type", "enum", "struct", "const", "let"].contains(&kw.as_str()) {
...
static STDLIB_EXPORTS_CACHE: std::sync::OnceLock<std::sync::Mutex<std::collections::HashMap<String, Vec<std::sync::Arc<str>>>>> = std::sync::OnceLock::new();

fn get_cached_exports(module_id: &str) -> Option<Vec<std::rc::Rc<str>>> {
    let cache_mutex = STDLIB_EXPORTS_CACHE.get_or_init(|| std::sync::Mutex::new(std::collections::HashMap::new()));
