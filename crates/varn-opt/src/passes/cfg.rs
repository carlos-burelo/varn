use crate::ssa::ir::{Block, BlockId, SsaFunc, Terminator, InstKind};

/// Run CFG simplification and compaction on `func`. Returns `true` if any changes were made.
pub fn simplify_and_compact(func: &mut SsaFunc) -> bool {
    let mut changed = false;
    
    // 1. Fold redundant branches (Branch where then_blk == else_blk)
    ...
    // 4. Compact CFG (always run if any changes occurred, or run to clean up unreachable blocks)
...
fn fold_redundant_branches(func: &mut SsaFunc) -> bool {
    ...
    for b_idx in 0..func.blocks.len() {
        ...
        if let Terminator::Branch { cond: _, then_blk, then_args, else_blk, else_args: _ } = term {
                    ...
                    for a_id in b_preds {
                        ...
                        match a_term {
                            Terminator::Jump { target, .. } if *target == b_id => {
                                updates.push((a_id, Terminator::Jump { target: c_id, args: args.clone() }));
                            }
                            Terminator::Branch { cond, then_blk, then_args, else_blk, else_args } => {
                                if *then_blk == b_id || *else_blk == b_id {
                                    ...
                                    let new_then_args = if *then_blk == b_id { args.clone() } else { then_args.clone() };
                                    let new_else = if *else_blk == b_id { c_id } else { *else_blk };
                                    let new_else_args = if *else_blk == b_id { args.clone() } else { else_args.clone() };
                                    updates.push((a_id, Terminator::Branch {
                                        cond: *cond,
                                        then_blk: new_then,
        ...
        if let Terminator::Jump { target: b_id, args } = term {
            if b_id != a_id {
                ...
                if b_preds == &[a_id] {
                    ...
                    let b_term = std::mem::replace(&mut func.blocks[b_id.0 as usize].term, Terminator::Unreachable);

                    // Implicit `Try` -> handler edges in the moved instructions now
                    // originate from `a_id`; repoint the handlers' predecessors.
