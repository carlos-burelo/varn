...
varn_contract! {
    ...
    impl TestRuntime {
        fn assert(_ctx: &mut dyn NativeCtx, label: &str, condition: bool) -> Result<(), String> {
            if condition {
                PASSED.fetch_add(1, Ordering::Relaxed);
            } else {
                FAILED.fetch_add(1, Ordering::Relaxed);
        ...
        fn assertEqual(ctx: &mut dyn NativeCtx, actual: VmValue, expected: VmValue, message: Option<&str>) -> Result<(), String> {
            if actual == expected {
                PASSED.fetch_add(1, Ordering::Relaxed);
            } else {
                FAILED.fetch_add(1, Ordering::Relaxed);
        ...
        fn summary(_ctx: &mut dyn NativeCtx) -> Result<(), String> {
            print_summary();
            Ok(())
        }
    }
