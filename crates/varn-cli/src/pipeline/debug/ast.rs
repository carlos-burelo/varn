...
fn print_stmt(stmt: &Stmt, indent: &str, is_last: bool) {
    ...
    match stmt {
        ...
        _ => terminal::log(format!("{indent}{marker}{}", chalk(format!("{:?}", stmt)).dim())),
    }
}
...
fn print_decl(decl: &Decl, indent: &str, is_last: bool) {
    ...
    match decl {
        Decl::Function(f) => {
            terminal::log(format!("{indent}{marker}{} {}", chalk("FunctionDecl").blue().bold(), chalk(&f.id).yellow()));
            print_stmt(&f.body, &child_indent, true);
        }
        Decl::Variable(v) => {
            terminal::log(format!("{indent}{marker}{} {}", chalk("VariableDecl").blue().bold(), chalk(format!("({:?})", v.kind)).dim()));
            for (i, d) in v.declarators.iter().enumerate() {
                ...
                terminal::log(format!("{child_indent}{d_marker}{} {}", chalk("Var").bold(), chalk(id_name).yellow()));
        ...
        _ => terminal::log(format!("{indent}{marker}{} {}", chalk("Decl").blue().bold(), chalk(format!("{:?}", decl)).dim())),
    }
}
...
fn print_expr(expr: &Expr, indent: &str, is_last: bool) {
    ...
    match expr {
        Expr::Identifier { name, .. } => {
            terminal::log(format!("{indent}{marker}{} {}", chalk(name).yellow(), chalk("(id)").dim()));
        }
        Expr::IntLiteral { value, .. } => {
            terminal::log(format!("{indent}{marker}{} {}", chalk(value).blue(), chalk("(int)").dim()));
        }
        Expr::FloatLiteral { value, .. } => {
            terminal::log(format!("{indent}{marker}{} {}", chalk(value).blue(), chalk("(float)").dim()));
        }
        Expr::StrLiteral { value, .. } => {
            terminal::log(format!("{indent}{marker}{} {}", chalk(format!("\"{value}\"")), chalk("(str)").dim()));
        }
        Expr::BoolLiteral { value, .. } => {
            terminal::log(format!("{indent}{marker}{} {}", chalk(value).blue(), chalk("(bool)").dim()));
        }
        Expr::NullLiteral { .. } => {
            terminal::log(format!("{indent}{marker}{} {}", chalk("null").blue(), chalk("(null)").dim()));
        }
        Expr::Binary { left, op, right, .. } => {
            terminal::log(format!("{indent}{marker}{} {}", chalk(format!("{:?}", op)).green(), chalk("(binary)").dim()));
            print_expr(left, &child_indent, false);
            print_expr(right, &child_indent, true);
        }
        Expr::Unary { op, operand, .. } => {
            terminal::log(format!("{indent}{marker}{} {}", chalk(format!("{:?}", op)).green(), chalk("(unary)").dim()));
            print_expr(operand, &child_indent, true);
        }
        Expr::Call { callee, args, .. } => {
            terminal::log(format!("{indent}{marker}{}", chalk("Call").bold()));
            ...
            for (i, arg) in args.iter().enumerate() {
                ...
                match arg {
                    ...
                    Arg::Named { label, value } => {
                        ...
                        terminal::log(format!("{child_indent}{a_marker}{}", chalk(format!("{label}:")).dim()));
                        let a_indent = format!("{child_indent}{}", if arg_is_last { "    " } else { "│   " });
                        print_expr(value, &a_indent, true);
                    }
                    Arg::Spread(e) => {
                        let a_marker = if arg_is_last { "└── " } else { "├── " };
                        terminal::log(format!("{child_indent}{a_marker}{}", chalk("...").dim()));
                        let a_indent = format!("{child_indent}{}", if arg_is_last { "    " } else { "│   " });
        ...
        Expr::Member { object, property, .. } => {
            terminal::log(format!("{indent}{marker}{}", chalk("Member").bold()));
            print_expr(object, &child_indent, false);
            print_expr(property, &child_indent, true);
        }
        _ => {
            terminal::log(format!("{indent}{marker}{}", chalk(format!("{:?}", expr)).dim()));
        }
    }
}
