...
thread_local! {
    static BUILTIN_PROTOS: RefCell<Option<Result<Vec<FunctionProto>, String>>> = RefCell::new(None);
}

pub fn builtin_protos_owned() -> PipelineResult<Vec<FunctionProto>> {
    let result = BUILTIN_PROTOS.with(|c| {
        let mut guard = c.borrow_mut();
        if guard.is_none() {
            *guard = Some(
                postcard::from_bytes(BUILTIN_PROTOS_BYTES)
