...
mod tests {
    ...
    #[test]
    fn short_global_drops_path_and_extension() {
        assert_eq!(
            short_global("C:/x/tests/31-stdlib-migration-test.vn::hash"),
            "31-stdlib-migration-test::hash"
        );
        assert_eq!(short_global("assert"), "assert");
    }

    #[test]
    fn truncate_middle_keeps_both_ends() {
        // Budget of 5 = 2 head + ellipsis + 2 tail.
        assert_eq!(truncate_middle("abcdefghij", 5), "ab…ij");
        assert_eq!(truncate_middle("abcdefghij", 5).chars().count(), 5);
        assert_eq!(truncate_middle("short", 10), "short");
    }

    #[test]
    fn truncate_middle_keeps_the_symbol_visible() {
        // The point of elliding the middle: two globals from the same module
        // must not collapse to the same string.
        let a = truncate_middle("31-stdlib-migration-test::hash", LABEL_WIDTH);
        let b = truncate_middle("31-stdlib-migration-test::hi", LABEL_WIDTH);
        assert_ne!(a, b);
    }

    #[test]
    fn short_path_strips_windows_prefix() {
        assert!(!short_path(r"\\?\C:\a\b\main.vn").starts_with(r"\\?\"));
    }
}
