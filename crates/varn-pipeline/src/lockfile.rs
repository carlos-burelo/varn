...
pub fn sync_lockfile(entry_file: &str, graph: &ModuleGraphArtifact) -> Result<(), PipelineError> {
    ...
    if !env_dir.exists() {
        std::fs::create_dir_all(&env_dir)
            .map_err(|e| PipelineError::fatal(format!("cannot create .vn directory: {e}")))?;
    }
...
fn write_lockfile(lock_path: &Path, lock: &Lockfile) -> Result<(), PipelineError> {
    let json = serde_json::to_string_pretty(lock)
        .map_err(|e| PipelineError::fatal(format!("cannot serialize lockfile: {e}")))?;
